...
public class DiscountedCumilativeGain {
    ...
    private int[] relevanceScores;
    private float[] discounted_gain_list;
    private float[] dcgains;
	...
	if (relevanceScores.length > 1) {
		...
		discounted_gain_list[i] = (float) (((float) relevanceScores[i] / Math.log(i + 1)) * Math.log(2));
    ...
    public void save() {
	try (PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(disCumGainFilepath, false)))) {
