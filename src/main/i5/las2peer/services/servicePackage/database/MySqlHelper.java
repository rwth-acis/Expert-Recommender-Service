...
public class MySqlHelper {
	...
	public static void createAndInsertSemanticTags(
			ConnectionSource connectionSrc) throws SQLException {
		Dao<SemanticTagEntity, Long> SemanticDao = DaoManager.createDao(
				connectionSrc, SemanticTagEntity.class);

		// Iterate all the posts and extract tags from them.
		Dao<Data, Long> postsDao = DaoManager.createDao(connectionSrc,
				Data.class);
	...
	public static List<UserEntity> getUserDAOs(ConnectionSource connectionSrc)
			throws SQLException {
		Dao<UserEntity, Long> userDao = DaoManager.createDao(connectionSrc,
				UserEntity.class);
	...
	public static void createUserMap(ConnectionSource connectionSrc)
			throws SQLException {
		Dao<UserEntity, Long> userDao = DaoManager.createDao(connectionSrc,
				UserEntity.class);
		List<UserEntity> user_entites = userDao.queryForAll();
		for (UserEntity entity : user_entites) {
			Global.userId2userObj1.put(entity.getUserId(), entity);
		}
	}
	...
	public static void createSemanticTagFreqMap(ConnectionSource connectionSrc)
			throws SQLException {
