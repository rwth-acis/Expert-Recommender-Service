...
public class ModelingStrategy1 implements ScoreStrategy {
	...
	public ModelingStrategy1(DbTextIndexer textIndexer,
			DbSematicsIndexer semanticsIndexer) {
		...
		try {
			for (Map.Entry entry : mTextIndexer.getTokenMap().entries()) {
				...
				if (mTextIndexer.getPostId2UserIdMap().containsKey(postid)) {
					long userid = mTextIndexer.getPostId2UserIdMap()
							.get(postid);
					...
					double sum = 0;
					double termFreq_inverseResFreq = mTextIndexer.getTfIrfMap()
							.get(postid);
					...
					if (mSemanticsIndexer.getEfirfMap().get(postid) != null) {
						entityFreq_inverseResFreq = mSemanticsIndexer
								.getEfirfMap()
								.get(postid);
					}
